...
function Shell() {
  const { activePage, isDark, largeText, highContrast, closeAllModals, language } = useApp();

  // Sync the dark-theme class onto <body>, same element the original
  // vanilla version toggled it on. The large-text and high-contrast
  // accessibility preferences ride the same mechanism — a single class on
  // <body> that the stylesheet keys its overrides off.
  useEffect(() => {
    document.body.classList.toggle('theme-dark', isDark);
  }, [isDark]);
  useEffect(() => {
    document.body.classList.toggle('theme-large-text', largeText);
  }, [largeText]);
  useEffect(() => {
    document.body.classList.toggle('theme-contrast', highContrast);
  }, [highContrast]);
